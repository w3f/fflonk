...
impl<F: PrimeField, C: Commitment<F>> Claim<F, C> {
    pub fn new<CS>(ck: &CS::CK, poly: &Poly<F>, at: F) -> Claim<F, C>
    where
        CS: PCS<F, C = C>,
    {
        Claim {
...
pub fn aggregate_claims<F: PrimeField, CS: PCS<F>>(
    claims: &[Claim<F, CS::C>],
    rs: &[F],
) -> Claim<F, CS::C> {
    ...
    let (rcs, rys): (Vec<CS::C>, Vec<F>) = claims
        .iter()
        .zip(rs.iter())
